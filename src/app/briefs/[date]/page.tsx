...
type BriefPageProps = { params: Promise<{ date: string }> };

export function generateStaticParams() {
  return briefs.map(({ date }) => ({ date }));
}

export async function generateMetadata({ params }: BriefPageProps): Promise<Metadata> {
  const { date } = await params;
  const brief = getBrief(date);
  if (!brief) return { title: "速递不存在", robots: { index: false, follow: false } };
  return { title: brief.title, description: brief.description };
}

const status = {
  full_text: { label: "已读取正文", icon: CheckCircle2 },
  title_only: { label: "仅读取标题", icon: CircleAlert },
} as const;

export default async function TencentBriefPage({ params }: BriefPageProps) {
  const { date } = await params;
  const brief = getBrief(date);
  if (!brief) notFound();
  const categories = [...new Set(brief.items.map(({ category }) => category))];
      ...
      <header className="border-b border-[color:var(--color-line)]">
        <div className="wrap py-14 sm:py-20">
          ...
          <div className="grid gap-10 lg:grid-cols-[1fr_18rem] lg:items-end">
            <div>
              <p className="eyebrow mb-5">NON-OFFICIAL AI BRIEF · {brief.date}</p>
              ...
              <section key={item.id} id={item.id} className="scroll-mt-24">
                {sectionStart && (
                  <div className="mt-16 flex items-center gap-4 border-b border-[color:var(--color-line-strong)] pb-3 first:mt-0">
                    <span className="eyebrow">SECTION {String(categories.indexOf(item.category) + 1).padStart(2, "0")}</span>
